...
Orca = AutoregressiveWrapper(Orca)
...
def train(args):
    ...
    dataset = dataset.map(Orca.decoder.tokenizer, batched=True,
    ...
    accelerator.init_trackers(project_name="Orca")

    #wandb
    wandb.init(project="Orca", config=args)
